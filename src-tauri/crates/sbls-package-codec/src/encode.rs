...
use crate::registry::{
    ...
    CURRENT_WRITABLE_PROJECT_SCHEMA_VERSION,
};
...
fn encode_project_package_inner_with_raster_budget_borrowed(
    ...
    if probed_schema_version != CURRENT_WRITABLE_PROJECT_SCHEMA_VERSION {
        return Err(failure(
    ...
    if schema_version != CURRENT_WRITABLE_PROJECT_SCHEMA_VERSION {
        return Err(failure(
            FailureCode::ProjectSchemaUnsupported,
            FailureStage::Project,
        ));
...
mod tests {
    ...
    fn raw_captures_for(
        ...
        expand_registered_owners("0.4.0", shape)
            .unwrap()
            .into_iter()
    ...
    fn disc_json(extra: &str) -> Vec<u8> {
        ...
        format!(
            r#"{{"schemaVersion":"0.4.0","projectType":"disc","template":{{"type":"disc"}}{steam_banner}{logo_assets}{rating_badge}{media_mark}{extra}}}"#
        )
        .into_bytes()
    }
    ...
    fn case_json(
        ...
        format!(
            r#"{{"schemaVersion":"0.4.0","projectType":"caseInsert","template":{{"type":"caseInsert"}},"caseInsert":{{"templates":{{"cover":{},"tray":{}}},"spine":{{"left":{},"right":{}}}}}}}"#,
            surface(cover_extra),
            surface(tray_extra),
    ...
    fn absent_case_surfaces_require_capture_of_normalized_default_banners() {
        let json = br#"{"schemaVersion":"0.4.0","projectType":"caseInsert","template":{"type":"caseInsert"},"caseInsert":{}}"#.to_vec();
    ...
    fn implicit_and_variable_semantic_built_ins_require_exact_capture_bytes() {
        let implicit_disc =
            br#"{"schemaVersion":"0.4.0","projectType":"disc","template":{"type":"disc"}}"#
                .to_vec();
    ...
    fn malformed_normalized_project_and_wrong_schema_fail_before_capture() {
        ...
        for previous in ["0.1.0", "0.2.0", "0.3.0"] {
            let source = format!(
                r#"{{"schemaVersion":"{previous}","projectType":"disc","template":{{"type":"disc"}}}}"#
            );
            assert_eq!(
                encode_project_package(&input(source.into_bytes(), Vec::new()))
                    .unwrap_err()
                    .code,
                FailureCode::ProjectSchemaUnsupported,
                "{previous}"
    ...
    fn project_kind_resolution_matches_current_schema_precedence_and_template_rules() {
        let editor_only = br#"{"schemaVersion":"0.4.0","editor":{"projectType":"disc"},"template":{"type":"disc"}}"#;
        ...
        let direct_wins = br#"{"schemaVersion":"0.4.0","projectType":"disc","editor":{"projectType":"caseInsert","workspace":"caseInsert"},"template":{"type":"disc"}}"#;
        ...
        for invalid in [
            br#"{"schemaVersion":"0.4.0","projectType":"other","template":{"type":"disc"}}"#.as_slice(),
            br#"{"schemaVersion":"0.4.0","projectType":"caseInsert","template":{"type":"disc"}}"#.as_slice(),
            br#"{"schemaVersion":"0.4.0","editor":{"workspace":"home"},"template":{"type":"disc"}}"#.as_slice(),
        ] {
        ...
        for template_type in ["caseInsert", "jewelCase", "dvdAmaray", "bluRay"] {
            let json = format!(
                r#"{{"schemaVersion":"0.4.0","projectType":"caseInsert","template":{{"type":"{template_type}"}},"caseInsert":{{}}}}"#
            );
    ...
    fn cross_kind_oversized_asset_paths_are_rejected_before_materialization() {
        ...
        for json in [
            format!(
                r#"{{"schemaVersion":"0.4.0","projectType":"caseInsert","template":{{"type":"caseInsert"}},"caseInsert":{{}},"background":{{"imageDataUrl":"{oversized}"}}}}"#
            ),
            format!(
                r#"{{"schemaVersion":"0.4.0","projectType":"disc","template":{{"type":"disc"}},"caseInsert":{{"templates":{{"cover":{{"background":{{"imageDataUrl":"{oversized}"}}}}}}}}}}"#
            ),
        ] {

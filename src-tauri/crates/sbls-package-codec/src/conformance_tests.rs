...
fn case_surface_project_json(surface_index: usize, catalog: &[RasterFixture]) -> String {
    let url = |index: usize| catalog[index % catalog.len()].data_url.as_str();
    let base = surface_index * 7;
    let reserved_artwork_viewport = if surface_index == 1 {
        r#","reservedArtworkViewport":{"kind":"sbls/case-insert-artwork-viewport","formatVersion":1,"templateId":"jewelCase","templateRevision":null,"coordinateBasis":"backPanelSafe","widthPercent":26,"heightPercent":16,"focalPosition":{"xPercent":50,"yPercent":50},"zoom":1}"#
    } else {
        ""
    };
    let artwork_slot = format!(
        r#"{{"enabled":false,"imageDataUrl":"{}","imageSource":{{"source":"uploaded"}}{reserved_artwork_viewport}}}"#,
        url(base + 4),
    );
    format!(
        r#"{{"steamBanner":{{"placement":"hidden","lockupImageDataUrl":"{}","lockupImageSource":{{"source":"custom"}}}},"background":{{"enabled":false,"remembered":true,"imageDataUrl":"{}","imageSource":{{"source":"uploaded"}}}},"titleArtwork":{{"enabled":false,"imageDataUrl":"{}","imageSource":{{"source":"custom"}},"defaultSteamLogo":{{"imageDataUrl":"{}","imageSource":{{"source":"steam-artwork"}}}}}},"artworkSlots":[{artwork_slot}],"logoSlots":[{{"enabled":false,"imageDataUrl":"{}","imageSource":{{"source":"custom"}}}}],"markSlots":[{{"enabled":false,"imageDataUrl":"{}","imageSource":{{"source":"custom"}}}}]}}"#,
        url(base),
        url(base + 1),
        url(base + 2),
        url(base + 3),
...
fn package_project_bytes(package: &[u8]) -> Vec<u8> {
    let inventory = inspect_zip32(package, &PackageLimits::V1)
        .unwrap()
        .validate_v1_layout()
        .unwrap();
    let (index, _) = inventory.find(PROJECT_ENTRY_NAME).unwrap();
    inventory
        .read_entry(
            index,
            EntryRole::Project,
...
fn full_disc_current_schema_public_round_trip_is_exact_and_deterministic() {
    ...
    assert_full_public_round_trip(
        full_disc_project("0.4.0", &catalog),
        disc_owner_specs(),
        &catalog,
        "0.4.0",
    );
...
fn full_case_all_surfaces_public_round_trip_is_exact_and_deterministic() {
    let _schedule = native::native_test_schedule_lock();
    let catalog = raster_catalog();
    let source = full_case_project("0.4.0", &catalog);
    let package = assert_full_public_round_trip(source, case_owner_specs(), &catalog, "0.4.0");

    let expected_viewport = canonical_json(
        br#"{"kind":"sbls/case-insert-artwork-viewport","formatVersion":1,"templateId":"jewelCase","templateRevision":null,"coordinateBasis":"backPanelSafe","widthPercent":26,"heightPercent":16,"focalPosition":{"xPercent":50,"yPercent":50},"zoom":1}"#,
    );
    let expected_viewport = parse_json_with_limits(&expected_viewport, &PackageLimits::V1).unwrap();
    let projection =
        parse_json_with_limits(&package_project_bytes(&package), &PackageLimits::V1).unwrap();
    let projected_artwork = projection
        .get("caseInsert")
        .and_then(|value| value.get("templates"))
        .and_then(|value| value.get("tray"))
        .and_then(|value| value.get("artworkSlots"))
        .and_then(|value| value.get_index(0))
        .unwrap();
    assert!(projected_artwork.get("imageDataUrl").unwrap().is_null());
    assert_eq!(
        projected_artwork.get("reservedArtworkViewport").unwrap(),
        &expected_viewport,
    );
    ...
    assert_eq!(
        ...
        Some(catalog[(7 + 4) % catalog.len()].data_url.as_str()),
    );
    assert_eq!(
        hydrated_artwork.get("reservedArtworkViewport").unwrap(),
        &expected_viewport,
    );
}
...
fn supported_older_schema_public_handoff_hydrates_without_migration() {
    ...
    for schema in ["0.1.0", "0.2.0", "0.3.0"] {
        ...
        let (package, _) = independent_package(&source, schema, &[asset], None);
        let decoded = decode_project_package(&package).unwrap();
        ...
        assert!(!String::from_utf8_lossy(decoded.hydrated_project_json())
            .contains(r#""schemaVersion":"0.4.0""#));
    }
}
...
fn golden_case_project(banner_value: &str, cover_background: Option<&str>) -> Vec<u8> {
    ...
    project.push_str(banner_value);
    project.push_str(
        r#"}}}},"projectType":"caseInsert","schemaVersion":"0.4.0","template":{"type":"caseInsert"}}"#,
    );
    project.into_bytes()
}
...
fn independent_raw_valid_golden_matches_public_writer_and_decoder() {
    ...
    let (golden, manifest) = independent_package(&projection, "0.4.0", &[asset], None);
...
fn zero_asset_manifest_and_store_envelope_have_exactly_two_entries() {
    let project = br#"{"caseInsert":{},"projectType":"caseInsert","schemaVersion":"0.4.0","template":{"type":"caseInsert"}}"#;
    let (package, manifest) = independent_package(project, "0.4.0", &[], None);
...
fn public_decode_jpeg_profile_code_yields_to_manifest_dimension_mismatch() {
    ...
    for (declared_width, expected) in [
        ...
        let (package, _) = independent_package(&project, "0.4.0", &[asset], None);
...
fn missing_bound_leaf_and_residual_package_tokens_are_rejected() {
    ...
    let (missing_leaf, _) =
        independent_package(&projection_without_background, "0.4.0", &[asset], None);
...
fn project_and_asset_declared_byte_lengths_are_verified_at_the_public_boundary() {
    ...
    let (asset_mismatch, _) = independent_package(&project, "0.4.0", &[length_mismatch], None);
...
fn nested_manifest_duplicate_and_closed_shape_matrix_is_rejected() {
    ...
    let (_, canonical) = independent_package(&project, "0.4.0", &[asset], None);
...
fn unknown_semantic_built_ins_are_rejected_at_both_public_facades() {
    let _schedule = native::native_test_schedule_lock();
    let catalog = raster_catalog();
    let disc = full_disc_project("0.4.0", &catalog);
    let case = full_case_project("0.4.0", &catalog);
    let disc_variants = [
        (
...
fn unbound_logo_evidence_and_direct_mark_absence_have_stable_public_precedence() {
    ...
    let case_source = replace_once(
        full_case_project("0.4.0", &catalog),

...
> Last reviewed against commit: `32e94b0a343d02bb7dfb74adb6d05d325cd73769`.
...
| [`APPLICATION_COMMAND_AND_PROJECT_LIFECYCLE_CONTRACT.md`](APPLICATION_COMMAND_AND_PROJECT_LIFECYCLE_CONTRACT.md) | Draft target-state normative contract | Target application-command semantics, single-project session lifecycle, dirty/baseline rules, replacement/close guards, and shared result/feedback boundaries. | Application commands, Home/editor project navigation, New/Open/Save/Save As/Close behavior, native close/Quit, global shortcuts, or lifecycle feedback work. | Target command/lifecycle conflicts defer to this contract; current as-built facts defer to source and the SDD; serialized schema conflicts defer to `PROJECT_FILE_SPEC.md`. |
| [`APPLICATION_MENU_BAR_CONTRACT.md`](APPLICATION_MENU_BAR_CONTRACT.md) | Draft target-state normative contract | Final target File/Edit/Tools/Window/Help hierarchy, stable presentation item IDs, native Tauri ownership/bridge, capability projection, workflow launchers, shortcuts/platform behavior, and sidebar migration gates. | Application-menu presentation or implementation, native menu events/state, File/Edit/Tools/Window/Help items, application shortcuts, workflow relocation, or Help/About work. | Target menu presentation and integration conflicts defer to this contract; lifecycle commands defer to the lifecycle contract; workflow/navigation semantics defer to their focused contracts; current as-built facts defer to source and the SDD. |
...
| [`EDITOR_NAVIGATION_AND_CONTROL_OWNERSHIP.md`](EDITOR_NAVIGATION_AND_CONTROL_OWNERSHIP.md) | Draft target-state normative reference | Target semantic control classes, presentation/owner boundaries, complete audited control ownership, typed editor destinations, focus routing, and navigation-state lifetimes. | Editor navigation, control relocation, Disc/Case surface routing, preview-to-control work, contextual hosts, Guided focus, or ownership-boundary changes. | Target navigation/control-ownership conflicts defer to this reference; application commands defer to the lifecycle contract; current as-built facts defer to source and the SDD; schema and rich-text conflicts defer to their focused contracts. |

...
| [`TEXT_EDITOR_SMOKE_AUTOMATION.md`](TEXT_EDITOR_SMOKE_AUTOMATION.md) | Conditional reference | Browser smoke automation guide and supported text-editor pilot paths. | Before browser automation against the text editor or updating `npm run smoke:text-editor`. | Automation-process conflicts defer to this guide; behavioral contracts defer to `TEXT_EDITOR_CONTRACT.md`. |

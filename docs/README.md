...
| [`ARTWORK_FRAME_MATERIAL_CONTRACT.md`](ARTWORK_FRAME_MATERIAL_CONTRACT.md) | Initial subsystem contract | Procedural additional-artwork frame material behavior, starting with steel. | Additional artwork frame materials, metal profiles, steel surface artifacts, or frame preview/export parity. | Material intent conflicts defer to this contract; preview/export architecture conflicts defer to the SDD. |
| [`STEEL_RUST_STAGE_MODEL.md`](STEEL_RUST_STAGE_MODEL.md) | Implementation planning contract | Staged rust progression for steel and black-iron frame aging. | Steel or black-iron `metalTarnish`, rust maps, rust lighting, flaking, exposed-metal masks, or rust-stage tests. | Steel/black-iron rust staging conflicts defer to this file; broader material behavior defers to the material contract; preview/export architecture defers to the SDD. |
| [`STEEL_POLISH_STAGE_MODEL.md`](STEEL_POLISH_STAGE_MODEL.md) | Implementation planning contract | Staged polish progression for steel and black-iron frame finish rendering. | Steel or black-iron `metalPolish`, brushed grain, scratches, dents, pits, roughness/gloss maps, anisotropic lighting, or polish-stage tests. | Steel/black-iron polish staging conflicts defer to this file; broader material behavior defers to the material contract; preview/export architecture defers to the SDD. |

## Temporary Working Plans

| Document | Status | Purpose | Read when | Conflict rule |
| --- | --- | --- | --- | --- |
| [`STEEL_POLISH_DECAL_REIMPLEMENTATION_PLAN.md`](STEEL_POLISH_DECAL_REIMPLEMENTATION_PLAN.md) | Temporary working plan | Small-stage plan for replacing steel polish defects with stage-owned physical decals so inactive scratches, dents, gouges, pits, and scuffs cannot cast shadows. | Before implementing the next photorealistic steel polish replacement stages. Delete after Stage 10 is implemented and durable contract language is folded into the steel polish and material contracts. | Current source and active contracts win for implemented behavior; this plan guides upcoming work only and must not be cited as implemented behavior. |

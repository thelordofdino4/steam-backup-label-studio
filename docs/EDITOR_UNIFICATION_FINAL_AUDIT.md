...
| Export warning copy | `src/export/preflightWarnings.ts` | Disc export preflight and case insert export preflight | Shared warning builders for guide export, missing images, layout values, unresolved fit/placement, and upscaling. Built-in/default app assets are valid output sources and must not create warnings solely because they are built in. |

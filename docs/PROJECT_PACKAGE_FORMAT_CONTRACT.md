...
| CURRENT FACT | Application sessions own a non-serialized `legacy-json`/`sbls-package-v1`/null discriminator plus the continuously synchronized normalized current project. Content-recognized Open, package Save/Save As, and New Disc/New Case are dispatcher-owned; New/Open use the shared dirty-aware guard, while Home Resume and Close/termination integration remain incomplete under issue #308. | [`projectSession.ts`](../src/lifecycle/projectSession.ts), [`appProjectLoad.ts`](../src/app/appProjectLoad.ts), [`appProjectNewCommand.ts`](../src/app/appProjectNewCommand.ts), [`appProjectOpenCommand.ts`](../src/app/appProjectOpenCommand.ts), [`appProjectReplacementGuard.ts`](../src/app/appProjectReplacementGuard.ts), [`appProjectSaveCommand.ts`](../src/app/appProjectSaveCommand.ts) |
...
| lifecycle/Open integration | CURRENT FACT / TARGET REQUIREMENT | All package work finishes before mutation; normalized project and restore candidate share one source; cancellation/failure/stale authorization preserves the aggregate; successful Open uses the shared dirty-aware guard and installs exact path/format/root identity/revision `0`/route/baseline/complete Disc or Case state. Home Resume and Close/termination integration remain future under #308. |

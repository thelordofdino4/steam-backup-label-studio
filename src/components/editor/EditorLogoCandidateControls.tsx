...
import type { LogoAssetKind } from '../../editor/logoAsset'
  ...
  return (
    <div className="logo-candidate-source-status-list">
      {discovery.sourceStatuses.map((sourceStatus) => (
        <p
          className="hint logo-candidate-source-status"
          key={`${sourceStatus.source}-${sourceStatus.label}`}
        >
...
export function EditorLogoCandidateControls({
  logoKey,
  label,
  discovery,
  ...
  return (
    <div className="logo-candidate-discovery">
      ...
      {discovery.error ? (
        <p className="hint logo-candidate-error">{discovery.error}</p>
      ) : null}
      {formatCandidateSourceStatus(discovery)}

      {!discovery.isLoading &&
      discovery.lastSearchedLabel &&
      discovery.candidates.length === 0 &&
      !discovery.error ? (
        <p className="hint">
          No logo candidates found for {discovery.lastSearchedLabel}. Manual

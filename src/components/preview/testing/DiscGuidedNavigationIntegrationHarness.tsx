...
const guidedLayout = getDiscGuidedLayoutDefinition(
  'disc:guided-layout:classic-top-title',
)

if (!guidedLayout) {
  throw new Error('Classic Top Title guided layout is required by the integration harness.')
}

const placeholders = guidedLayout.slotOrder.map((slotId) => {
...
function GuidedNavigationHarnessContent() {
  ...
  return (
    <main data-feature-snapshot={JSON.stringify(featureSnapshot)}>
      ...
      <section aria-label="Guided preview actions">
        <DiscGuidedPlaceholderActions placeholders={placeholders} />
      </section>
    </main>
  )
}

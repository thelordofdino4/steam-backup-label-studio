...
test('every setup popover is an accessible menu with Escape dismissal and focus return', () => {
  ...
  assert.match(actionsSource, /Remove from layout/)
  assert.match(actionsSource, /aria-label=\{`Remove \$\{actionViewModel\.label\} from layout`\}/)
  assert.doesNotMatch(actionsSource, /Remove from preset/)

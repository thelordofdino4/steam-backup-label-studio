...
function getRatingSetupAction() {
  ...
  assert.equal(setup.actions.length, 1)
  return setup.actions[0]
}
...
test('blank Classic exposes eight exact action models', () => {
  ...
  assert.ok(actions.every(({ setup }) =>
    setup.kind === 'menu' &&
    Object.isFrozen(setup) &&
    Object.isFrozen(setup.actions)))
})
test('Game Title remains the only two-action setup menu', () => {
  const title = getDiscGuidedPlaceholderSetup('game-title-choice')
  assert.equal(title.kind, 'menu')
  ...
  for (const kind of PLACEHOLDERS.map(([, , setupKind]) => setupKind)) {
    ...
    assert.equal(
      ...
      kind === 'game-title-choice' ? 2 : 1,
    )
  }
})
test('existing exact setup menu targets remain independent', () => {
  const expectations = [
  ...
  for (const [kind, destination] of expectations) {
    ...
    assert.equal(setup.kind, 'menu')
    assert.equal(setup.actions.length, 1)
    assert.deepEqual(setup.actions[0].request.destination, destination)
    assert.equal(setup.actions[0].request.scrollAlignment, 'role-start')
  }
})
...
test('every section-start setup menu dispatches its exact typed destination', () => {
  const expectations = [
    [
      'media-format-mark',
  ...
  for (const [kind, label, focusTarget, sectionAlignmentTarget] of
    ...
    assert.equal(setup.kind, 'menu')
    assert.equal(setup.actions.length, 1)
    assert.equal(setup.actions[0].label, label)
    assert.deepEqual(setup.actions[0].request, {
      surfaceId: 'disc-label',
      behavior: 'focus',
...
test('suggested placeholders retain exact setup without accepting content', () => {
  ...
  assert.deepEqual(
    actions.map(({ setup }) => setup.kind),
    Array.from({ length: 8 }, () => 'menu'),
  )
})

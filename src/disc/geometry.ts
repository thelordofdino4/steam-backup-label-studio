...
function getFiniteDiscLayoutPoint(point: LayoutPoint): LayoutPoint {
  return {
    x: Number.isFinite(point.x) ? point.x : DISC_LAYOUT_CENTER_PERCENT,
    y: Number.isFinite(point.y) ? point.y : DISC_LAYOUT_CENTER_PERCENT,
  }
}
...
export function getRectDistanceFromDiscCenter(
  ...
  const { x, y } = getFiniteDiscLayoutPoint(point)

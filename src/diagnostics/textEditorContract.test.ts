...
test('case insert inline editing uses the adapter input path', () => {
  ...
  assert.match(previewControls, /label:\s*'Bold'/)
  assert.match(previewControls, /label:\s*'Italic'/)
  assert.match(previewControls, /label:\s*'Underline'/)
...
test('curved disc text is not routed through a visible rectangular editor layer', () => {
  ...
  assert.match(adapter, /label:\s*'Bold'/)
  assert.match(adapter, /label:\s*'Italic'/)

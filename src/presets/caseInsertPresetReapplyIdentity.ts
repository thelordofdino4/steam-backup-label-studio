...
export function encodeCaseInsertPresetDeterministicIdentity(
  ...
  if (Array.isArray(value)) {
    const encoded = value.map(encodeCaseInsertPresetDeterministicIdentity).join('')
    return `a${value.length}:${encoded.length}:${encoded}`
  }
  if (isRecord(value)) {
    const entries = Object.keys(value).sort().map((key) =>
      encodeCaseInsertPresetDeterministicIdentity(key) +
      encodeCaseInsertPresetDeterministicIdentity(value[key]))
